...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const validInputs = inputTexts.filter((t: string) => t.trim().length > 0);
    if (validInputs.length === 0) {
        return res.status(200).json({ items: [] });
    }

    const prompt = `请处理以下商品列表：\n${validInputs.map((text: string, idx: number) => `${idx + 1}. ${text}`).join('\n')}`;
    ...
    return res.status(500).json({ error: "生成失败，请检查网络或 API Key。" });
  }
}
